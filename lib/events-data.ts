...
export const SHEET_CSV_URL =
  "https://docs.google.com/spreadsheets/d/e/2PACX-1vTff6oSbTkORJO5kPlGdFQfDnYDO3EquS5HQuH-Y9So7SsG23yJJO4zIOONxuiBdK1lgZ53-yvxCxff/pub?gid=1998048077&single=true&output=csv";

// Helper to parse CSV lines with quotes safely
...
export async function fetchLiveEvents(): Promise<FestivalEvent[]> {
  try {
    const res = await fetch(SHEET_CSV_URL, { cache: "no-store" });
    if (!res.ok) return FESTIVAL_EVENTS;

    const csvText = await res.text();
    const rows = csvText.trim().split(/\r?\n/);
    if (rows.length <= 1) return FESTIVAL_EVENTS;

    const headers = parseCsvLine(rows[0]);
    const loadedEvents: FestivalEvent[] = [];
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      loadedEvents.push({
        id: row.id,
        name: row.name,
        stageType: (row.stageType as StageType) || "on-stage",
        category: (row.category as EventCategory) || "Music",
        type: (row.type as "solo" | "group" | "duet") || "solo",
        gender: (row.gender as any) || "open",
        duration: row.duration || undefined,
        participantsCount: row.participantsCount || undefined,
        accompanistsCount: row.accompanistsCount || undefined,
        settingTime: row.settingTime || undefined,
        guidelines: row.guidelines
          ? row.guidelines.split("|").map((g) => g.trim()).filter(Boolean)
          : [],
      });
    }

    return loadedEvents.length > 0 ? loadedEvents : FESTIVAL_EVENTS;
  } catch (err) {
    console.warn("Could not load Google Sheet, falling back to local events:", err);
